...
import hCSS from './highlightDropdown.css';
import MLIcon from 'ml-react-cdl-icons';
import MarkerOrb from '../MarkerOrb/MarkerOrb.js';

let currentNoteText = '';

class HighlightDropdown extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...props,
  ...
  render() {
    const { highlightVisible, highlightPosition:hPos, highlightEvents, highlightData, markerObject, orientation, selectedMarkerId } = this.props;
    const { noteVisible, textareaValue } = this.state;
    let markerArray = [];
    let that = this;
      ...
      return (
        <div style={ { left: hPos.x - 10, top: hPos.y } } className={ hCSS['container'] }>
          ...
          <div className={ noteVisible ? hCSS['comment-wrap'] : hCSS['hidden'] }>
            <textarea className={ hCSS['comment-textarea']} onChange={ this._textareaChange } ></textarea>
...
HighlightDropdown.defaultProps = {
  markerObject: {},
}
HighlightDropdown.propTypes = {
